...
- **Topic:** 1.1 Economic Machine Template.
...
SUBSECTION
  ...
  IN-SCOPE:          Transactions-based GDP identity (P × Q = M × V); productivity growth as linear trend; the concept of short + long debt cycles as overlays on productivity; money vs credit distinction; the three-driver framework (productivity, short-term debt cycle, long-term debt cycle) as Dalio's master macro model.
  OUT-OF-SCOPE:      Mechanics of either debt cycle — covered by 1.2 and 1.3; deleveraging dynamics — covered by 1.4; inflation specifics — covered by 1.7.
...
NAMED FRAMEWORK COMPONENTS (TOPIC-BOUND, R17 ENFORCEMENT)
  ...
  1. **three drivers** — items: productivity growth; short-term debt cycle; long-term debt cycle
     Dalio anchor: HEMW (Economic Machine Works), 2012, p. 1-3
     Operationalization required: §5 must define a transform per driver; §6 must emit a regime tag dimension per driver; §7 must show one case per driver state; §11 must row each driver.
  2. **GDP identity components** — items: money; credit; transactions; goods/services
     Dalio anchor: HEMW, 2012, p. 1-2
     Operationalization required: §5 must define M, V, P, Q transforms; §11 must row each.
...
DELIVERABLE
  ...
  research_v2/01_economic_machine.md. Document title (the H1 at the top of the
  ...
  research_v2/01_economic_machine_quote_audit.md per R21.
...
HARD RULES (every rule binding; one violation = rejection)
  ...
  R21. VERBATIM QUOTE AUDIT APPENDIX (BLOCKING). In addition to the
       ...
       `research_v2/01_economic_machine_quote_audit.md` with this exact
       structure:

           # Quote Audit — 1.1 Economic Machine Template
...
After the model writes its main deliverable to `research_v2/01_economic_machine.md` and the quote-audit appendix to `research_v2/01_economic_machine_quote_audit.md`, run these checks IN ORDER:
...
11. **Side-by-side diff.** Compare structurally + substantively against `research/01_economic_machine.md` (existing version).
...
| Slot | Source | Format |
|---|---|---|
| `1.1` | registry entry `id` | string, e.g. `1.4` |
| `Economic Machine Template` | registry entry `title` | string |
| `01` | registry entry `seq` | string, e.g. `04` |
| `economic_machine` | registry entry `slug` | string |
| `Transactions-based GDP identity (P × Q = M × V); productivity growth as linear trend; the concept of short + long debt cycles as overlays on productivity; money vs credit distinction; the three-driver framework (productivity, short-term debt cycle, long-term debt cycle) as Dalio's master macro model.` | registry entry `scope_in` | prose paragraph |
| `Mechanics of either debt cycle — covered by 1.2 and 1.3; deleveraging dynamics — covered by 1.4; inflation specifics — covered by 1.7.` | registry entry `scope_out` | prose paragraph |
| `  1. **three drivers** — items: productivity growth; short-term debt cycle; long-term debt cycle
     Dalio anchor: HEMW (Economic Machine Works), 2012, p. 1-3
     Operationalization required: §5 must define a transform per driver; §6 must emit a regime tag dimension per driver; §7 must show one case per driver state; §11 must row each driver.
  2. **GDP identity components** — items: money; credit; transactions; goods/services
     Dalio anchor: HEMW, 2012, p. 1-2
     Operationalization required: §5 must define M, V, P, Q transforms; §11 must row each.` | registry entry `named_components` | bulleted list (one bullet per component, each with name + Dalio source page) |
| `    - US 1929-1932 (productivity-up + debt-cycle-down)
    - US 1990s (productivity-up + healthy expansion)
    - Japan 1990-2010 (productivity-flat + long-cycle-deleverage)
    - US 2008-2014 (post-crisis expansion)
    - China 2010-2020 (mid-cycle expansion)` | registry entry `expected_cases.allowlist` | bulleted list (one bullet per case) |
| `3` | registry entry `expected_cases.min` | integer literal |
| `    - How the Economic Machine Works (HEMW), 2012 paper + 30-min video
    - Big Debt Crises (BDC), 2018, Part 1 archetype
    - Principles for Navigating Big Debt Cycles (HCGB-1), 2024-2025, Ch 1
    - LinkedIn long-form essays on the three drivers` | registry entry `tier1_sources.allowlist` | bulleted list (one bullet per source) |
