...
SUBSECTION
  ...
  IN-SCOPE:          10-year regime-change detection; Dalio's 2019 framework for spotting reversals; asset-class leadership rotation by decade; the principle that each decade's winners reverse in the next.
  OUT-OF-SCOPE:      Empire-scale 250-year transitions — covered by 1.6; inflation-specific regime — covered by 1.7; cycle-level detection — covered by 1.2-1.4.
...
NAMED FRAMEWORK COMPONENTS (TOPIC-BOUND, R17 ENFORCEMENT)
  ...
  1. **paradigm-shift signals** — items: asset-class leadership rotation; valuation extremes; money-printing reaction
     Dalio anchor: Paradigm Shifts (LinkedIn), July 2019
     Operationalization required: §5 must define each signal as a transform; §6 must emit a paradigm-shift regime tag; §7 must show ≥3 decade-transition cases.
...
DELIVERABLE
  ...
  research_v2/05_paradigm_shifts.md. Document title (the H1 at the top of the
  ...
  research_v2/05_paradigm_shifts_quote_audit.md per R21.
...
HARD RULES (every rule binding; one violation = rejection)
  ...
  R21. VERBATIM QUOTE AUDIT APPENDIX (BLOCKING). In addition to the
       ...
       `research_v2/05_paradigm_shifts_quote_audit.md` with this exact
       structure:

           # Quote Audit — 1.5 Paradigm Shifts
...
After the model writes its main deliverable to `research_v2/05_paradigm_shifts.md` and the quote-audit appendix to `research_v2/05_paradigm_shifts_quote_audit.md`, run these checks IN ORDER:
...
11. **Side-by-side diff.** Compare structurally + substantively against `research/05_paradigm_shifts.md` (existing version).
...
| Slot | Source | Format |
|---|---|---|
| `1.5` | registry entry `id` | string, e.g. `1.4` |
| `Paradigm Shifts` | registry entry `title` | string |
| `05` | registry entry `seq` | string, e.g. `04` |
| `paradigm_shifts` | registry entry `slug` | string |
| `10-year regime-change detection; Dalio's 2019 framework for spotting reversals; asset-class leadership rotation by decade; the principle that each decade's winners reverse in the next.` | registry entry `scope_in` | prose paragraph |
| `Empire-scale 250-year transitions — covered by 1.6; inflation-specific regime — covered by 1.7; cycle-level detection — covered by 1.2-1.4.` | registry entry `scope_out` | prose paragraph |
| `  1. **paradigm-shift signals** — items: asset-class leadership rotation; valuation extremes; money-printing reaction
     Dalio anchor: Paradigm Shifts (LinkedIn), July 2019
     Operationalization required: §5 must define each signal as a transform; §6 must emit a paradigm-shift regime tag; §7 must show ≥3 decade-transition cases.` | registry entry `named_components` | bulleted list (one bullet per component, each with name + Dalio source page) |
| `    - 1920s reversal into 1930s (asset reversals + Great Depression)
    - 1970s reversal into 1980s (commodities to disinflation)
    - 2000s reversal into 2010s (post-GFC zero-rate paradigm)
    - 2020s emerging paradigm (debasement / new Cold War)` | registry entry `expected_cases.allowlist` | bulleted list (one bullet per case) |
| `3` | registry entry `expected_cases.min` | integer literal |
| `    - Paradigm Shifts (LinkedIn essay), July 2019
    - Principles for Dealing with the Changing World Order (CWO), 2021
    - Big Debt Crises (BDC), 2018
    - LinkedIn essays 2019-present on paradigm shifts` | registry entry `tier1_sources.allowlist` | bulleted list (one bullet per source) |
