...
- **Topic:** 1.7 Inflation & Currency Debasement.
...
SUBSECTION
  ...
  IN-SCOPE:          Monetary vs credit inflation distinction; gold and real-asset allocation under debasement; FX positioning; 'cash is trash' framing; real-rate regime classification; hyperinflation tail vs moderate-inflation regime.
  OUT-OF-SCOPE:      Long-term debt cycle generic — covered by 1.3; deleveraging levers generic — covered by 1.4.
...
NAMED FRAMEWORK COMPONENTS (TOPIC-BOUND, R17 ENFORCEMENT)
  ...
  1. **four inflation regimes** — items: moderate (real rate positive); high (real rate negative); debasement (M0 surge + FX collapse); hyperinflation (M0 logarithmic)
     Dalio anchor: HEMW, 2012, p. 5 + LinkedIn essays on cash/inflation
     Operationalization required: §5 must define classifier transforms; §6 must emit regime tag; §7 must show one case per regime.
...
DELIVERABLE
  ...
  research_v2/07_inflation_currency.md. Document title (the H1 at the top of the
  ...
  research_v2/07_inflation_currency_quote_audit.md per R21.
...
HARD RULES (every rule binding; one violation = rejection)
  ...
  R21. VERBATIM QUOTE AUDIT APPENDIX (BLOCKING). In addition to the
       ...
       `research_v2/07_inflation_currency_quote_audit.md` with this exact
       structure:

           # Quote Audit — 1.7 Inflation & Currency Debasement
...
After the model writes its main deliverable to `research_v2/07_inflation_currency.md` and the quote-audit appendix to `research_v2/07_inflation_currency_quote_audit.md`, run these checks IN ORDER:
...
11. **Side-by-side diff.** Compare structurally + substantively against `research/07_inflation_currency.md` (existing version).
...
| Slot | Source | Format |
|---|---|---|
| `1.7` | registry entry `id` | string, e.g. `1.4` |
| `Inflation & Currency Debasement` | registry entry `title` | string |
| `07` | registry entry `seq` | string, e.g. `04` |
| `inflation_currency` | registry entry `slug` | string |
| `Monetary vs credit inflation distinction; gold and real-asset allocation under debasement; FX positioning; 'cash is trash' framing; real-rate regime classification; hyperinflation tail vs moderate-inflation regime.` | registry entry `scope_in` | prose paragraph |
| `Long-term debt cycle generic — covered by 1.3; deleveraging levers generic — covered by 1.4.` | registry entry `scope_out` | prose paragraph |
| `  1. **four inflation regimes** — items: moderate (real rate positive); high (real rate negative); debasement (M0 surge + FX collapse); hyperinflation (M0 logarithmic)
     Dalio anchor: HEMW, 2012, p. 5 + LinkedIn essays on cash/inflation
     Operationalization required: §5 must define classifier transforms; §6 must emit regime tag; §7 must show one case per regime.` | registry entry `named_components` | bulleted list (one bullet per component, each with name + Dalio source page) |
| `    - US 1970s stagflation
    - Weimar Germany 1922-1923 hyperinflation
    - US 2009-2014 zero-bound moderate
    - Argentina 2010-present chronic high inflation
    - Japan 1990-present near-zero inflation` | registry entry `expected_cases.allowlist` | bulleted list (one bullet per case) |
| `3` | registry entry `expected_cases.min` | integer literal |
| `    - How the Economic Machine Works (HEMW), 2012
    - Big Debt Crises (BDC), 2018, hyperinflation case studies
    - LinkedIn essays on cash debasement / fiat decay 2019-present
    - An In-Depth Look at Deleveragings, 2012 (inflationary archetype)` | registry entry `tier1_sources.allowlist` | bulleted list (one bullet per source) |
