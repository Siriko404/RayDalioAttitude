# Deep Research Prompt — 2.3 Alpha Generation & Portable Alpha
...
DELIVERABLE
  ...
  research_v2/10_alpha_portable_alpha.md. Document title (the H1 at the top of the
  ...
  research_v2/10_alpha_portable_alpha_quote_audit.md per R21.
...
HARD RULES (every rule binding; one violation = rejection)
  ...
  R21. VERBATIM QUOTE AUDIT APPENDIX (BLOCKING). In addition to the
       ...
       structure:
           ...
           # Quote Audit — 2.3 Alpha Generation & Portable Alpha
...
After the model writes its main deliverable to `research_v2/10_alpha_portable_alpha.md` and the quote-audit appendix to `research_v2/10_alpha_portable_alpha_quote_audit.md`, run these checks IN ORDER:
...
11. **Side-by-side diff.** Compare structurally + substantively against `research/10_alpha_portable_alpha.md` (existing version).
...
| Slot | Source | Format |
|---|---|---|
| `2.3` | registry entry `id` | string, e.g. `1.4` |
| `Alpha Generation & Portable Alpha` | registry entry `title` | string |
| `10` | registry entry `seq` | string, e.g. `04` |
| `alpha_portable_alpha` | registry entry `slug` | string |
| `Alpha / beta separation; information ratio; bet sizing; pure alpha vs traditional alpha; portable alpha construction (transport alpha from one beta to another); alpha decay; alpha-stream breadth and concentration limits.` | registry entry `scope_in` | prose paragraph |
| `Beta construction — covered by 2.2; leverage sizing — covered by 2.4.` | registry entry `scope_out` | prose paragraph |
| `  1. **alpha-beta separation framework** — items: beta sleeve; alpha overlay; tracking error budget
     Dalio anchor: Engineering Targeted Returns and Risks, 2011, p. 11 + p. 9
     Operationalization required: §5 must define separation transforms; §6 must emit alpha-decomposition vector; §7 must show worked examples.
  2. **information ratio formula** — items: IC; breadth; IR
     Dalio anchor: Engineering 2011 (cross-references Grinold-Kahn breadth identity, NON-DALIO)
     Operationalization required: §5 must define IR formula with NON-DALIO Grinold-Kahn anchor; §6 must emit IR per stream output.` | registry entry `named_components` | bulleted list (one bullet per component, each with name + Dalio source page) |
| `    - Bridgewater Pure Alpha worked example
    - Tracking-error 3% / 6% client examples (Engineering 2011 L466-467)
    - 60/40 portable-alpha overlay illustration` | registry entry `expected_cases.allowlist` | bulleted list (one bullet per case) |
| `3` | registry entry `expected_cases.min` | integer literal |
| `    - Engineering Targeted Returns and Risks, 2011
    - Principles 2017 (commercial; R9 fair-use only)
    - Bridgewater Daily Observations on alpha (free citations)
    - LinkedIn essays on alpha generation` | registry entry `tier1_sources.allowlist` | bulleted list (one bullet per source) |
