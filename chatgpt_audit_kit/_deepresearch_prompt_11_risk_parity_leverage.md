...
- **Topic:** 2.4 Risk Parity & Leverage.
...
SUBSECTION
  ...
  IN-SCOPE:          Vol-targeted weighting; leverage ratios to hit target return; funding-cost analysis; rebalancing cadence; leverage limits; the principle that risk-parity portfolios require leverage to match return targets of capital-weighted portfolios.
  OUT-OF-SCOPE:      Asset-class weights per se — covered by 2.2; alpha-specific sizing — covered by 2.3.
...
NAMED FRAMEWORK COMPONENTS (TOPIC-BOUND, R17 ENFORCEMENT)
  ...
  1. **risk-parity construction steps** — items: target volatility; asset volatility estimation; weight calculation; leverage application
     Dalio anchor: Our Thoughts About Risk Parity and All-Weather, 2015 + Engineering 2011
     Operationalization required: §5 must define each step as a transform; §6 must emit leverage ratio; §7 must show worked examples.
...
DELIVERABLE
  ...
  research_v2/11_risk_parity_leverage.md. Document title (the H1 at the top of the
  ...
  research_v2/11_risk_parity_leverage_quote_audit.md per R21.
...
HARD RULES (every rule binding; one violation = rejection)
  ...
  R21. VERBATIM QUOTE AUDIT APPENDIX (BLOCKING). In addition to the
       ...
       `research_v2/11_risk_parity_leverage_quote_audit.md` with this exact
       structure:

           # Quote Audit — 2.4 Risk Parity & Leverage
...
After the model writes its main deliverable to `research_v2/11_risk_parity_leverage.md` and the quote-audit appendix to `research_v2/11_risk_parity_leverage_quote_audit.md`, run these checks IN ORDER:
...
11. **Side-by-side diff.** Compare structurally + substantively against `research/11_risk_parity_leverage.md` (existing version).
...
| Slot | Source | Format |
|---|---|---|
| `2.4` | registry entry `id` | string, e.g. `1.4` |
| `Risk Parity & Leverage` | registry entry `title` | string |
| `11` | registry entry `seq` | string, e.g. `04` |
| `risk_parity_leverage` | registry entry `slug` | string |
| `Vol-targeted weighting; leverage ratios to hit target return; funding-cost analysis; rebalancing cadence; leverage limits; the principle that risk-parity portfolios require leverage to match return targets of capital-weighted portfolios.` | registry entry `scope_in` | prose paragraph |
| `Asset-class weights per se — covered by 2.2; alpha-specific sizing — covered by 2.3.` | registry entry `scope_out` | prose paragraph |
| `  1. **risk-parity construction steps** — items: target volatility; asset volatility estimation; weight calculation; leverage application
     Dalio anchor: Our Thoughts About Risk Parity and All-Weather, 2015 + Engineering 2011
     Operationalization required: §5 must define each step as a transform; §6 must emit leverage ratio; §7 must show worked examples.` | registry entry `named_components` | bulleted list (one bullet per component, each with name + Dalio source page) |
| `    - All-Weather risk-parity worked example (Engineering 2011 ~2x leverage)
    - 1970s rising-rates leverage stress
    - 2008 funding-cost spike` | registry entry `expected_cases.allowlist` | bulleted list (one bullet per case) |
| `3` | registry entry `expected_cases.min` | integer literal |
| `    - Our Thoughts About Risk Parity and All-Weather, 2015
    - Engineering Targeted Returns and Risks, 2011
    - The All-Weather Story
    - LinkedIn essays on leverage / risk parity` | registry entry `tier1_sources.allowlist` | bulleted list (one bullet per source) |
