...
The generator script `_deepresearch_prompt_generator.py` substitutes the following slots. The registry is content-free (only structural identifiers per topic) and so is the slot list — `<<<SUBSECTION_MAP>>>` is the only block-shaped slot, computed by the generator from all 12 entries' (id, title) pairs:
...
| `<<<SUBSECTION_MAP>>>` | computed from all 12 registry entries | indented Markdown list; current subsection marked with `← THIS SUBSECTION` |
